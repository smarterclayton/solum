...
class DomainPersistentObject(object):
...
def remotable_classmethod(fn):
    ...
    def wrapper(cls, context, *args, **kwargs):
        if DomainObject.indirection_api:
            result = DomainObject.indirection_api.object_class_action(
...
def remotable(fn):
    ...
    def wrapper(self, *args, **kwargs):
        ctxt = self._context
        try:
            if isinstance(args[0], (rpc_common.CommonRpcContext)):
                ctxt = args[0]
                args = args[1:]
        except IndexError:
            pass
        ...
        self._context = ctxt
        if DomainObject.indirection_api:
            updates, result = DomainObject.indirection_api.object_action(
...
class DomainObjectSerializer(solum.openstack.common.rpc.serializer.Serializer):
    """A DomainObject-aware Serializer.

    This implements the Oslo Serializer interface and provides the
    ability to serialize and deserialize DomainObject entities. Any service
    that needs to accept or return DomainObjects as arguments or result values
    ...
    def deserialize_entity(self, context, entity):
        if isinstance(entity, dict) and 'nova_object.name' in entity:
            entity = DomainObject.obj_from_primitive(entity, context=context)
